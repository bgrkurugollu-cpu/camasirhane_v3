...
6. **Loglama:** `app/logger.py` structlog JSON üretir. Operasyonel müdahale gerektiren olaylar (`category="critical"`) için `log_critical()` kullanılır; alerting aggregation katmanında bu alana göre kurulur (bkz. `topoloji.md §13`). Dosya yükleme yalnızca `users/me/photo` ucundadır ve magic-byte + boyut doğrulamalıdır (`topoloji.md §7.1`); yeni yükleme yüzeyi eklenirse aynı denetimler zorunludur.
